...
+'<i>если одна строка, то: <br>'
+'let i=3;while(i)<br>alert(i--);//3,2,1</i></li>'
+'<li><b>do..while</b> (проверка под телом цикла)<br>'
+'let i=0; do{<br>'
	+'alert(i); <br>'
	+'i++;<br>'
+'}while(i<3); <i>//0,1,2</i><br></li>'
+'<li><b>Цикл "for"</b> самый распространённый<br>'
+'for(let i=0;i<3;i++){<br>'
+'   alert(i);} <i>//0,1,2</i><br>'
+'<table border="1"><caption>конструкция for</caption>'
+'<tr><td colspan="3"><b>Части</b></td></tr>'
+'<tr><td>начало</td><td>let i=0;</td><td>выполняется один раз '
+'при входе в цикл</td></tr>'
+'<tr><td>условие</td><td>i<3</td><td>проверяется перед каждой '
+'итерацией цикла. Если оно вычисляется в false, цикл остановится.</td></tr>'
+'<tr><td>тело</td><td>alert(i)</td><td>выполняется снова и снова, '
+'пока условие вычисляется в true.</td></tr>'
+'<tr><td>шаг</td><td>i++</td><td>Выполняется после тела цикла '
+'на каждой итерации перед проверкой условия.</td></tr>'
+'</table></i>'
+'<li><i>втроенное объявление переменной</i><br>'
+'for(let i=0;i<3;i++){<br>'
	+'alert(i); <i>//0,1,2(внутри)</i><br>'
+'}alert(i); <i>//вообще ничего не загрузила(снаружи)</i><br>'
+'<i>готовая переменная(уже объявлена)</i><br>'
+'let i=0;<br>'
+'for(i=0;i<3;i++){ <i>//не объявляем по новой</i><br>'
  +' alert(i); <i>//0,1,2</i><br>'
+'}alert(i); <i>//3 (поворот)</i><br></li>'
+'<li><b>Пропуск частй "for"</b><br>'
+'<i>Начало: </i><br>'
+'let i=0; <i>//уже объявили</i><br>'
+'for(;i<3;i++){<i> //пропустили</i><br>'
+'alert(i);<br>'
+'} <i>//0,1,2 (как мило)</i><br>'
+'<i>уберём шаг: </i><br>'
+'let i=0;<br>'
+'for(;i<3;){<br>'
+'alert(i++); }	<i>//0,1,2</i><br></li>'
+'<li><a>Прерывание цикла:"break"</a><br>'
+'let sum=0;<br>'
+'while(true){<br>'
	+'let value=+prompt("chislo",""); <i>//2,3,4,a</i><br>'
	+'if(!value) break; <i>//если не число, то выход</i><br>'
  +'  sum+=value; <br>'
+'}alert("Summa: "+sum); <i>//9 и после а выход</i><br></li>'
+'<li><b>Переход к следующей итерации: continue</b><br>'
+'<i>continue - это "облегчённая версия" break.</i><br>'
+'for(let i=0;i<10;i++){<br>'
	+'if(i%2==0) continue; <i>//пропускаются всё чётные цифры</i><br>'
+'alert(i);} <i>//1,3,5,7,9</i>	<br>'
+'<i>директива позволяет избегать вложенности(чтобы это не значало)</i><br>'
+'for(let i=0;i<10;i++){<br>'
	+'if(i%3){<br>'
+'alert(i);}} <i>//1,2,4,5,7,8</i><br>'
+'<a>Справа от оператора ? break/continue использовать нельзя</a><br>'
+'outer: for(let i=0;i<3;i++){//!<br>'
	+'for(let j=0;j<3;j++){<br>'
		+'let input=prompt(`Znach na Coord{${i},${j}}`,"");<br>'
	+'	if(!input) break outer; <i>//ничего не вводишь, то сразу выход</i><br>'
+'}}alert("Gotovo!");<br></li>'
+'<b>Итог</b> - <a>есть три вида цикла:</a><br>'
+'<ul><li type="disc">while - проверяет условие перед каждой итерацией.</li>'
+'<li type="disc">do...while - проверяет условие после каждой итерации</li>'
+'<li type="disc">for(;;) - проверяет условие перед каждой итерацией '
+'есть возможность задать дополнительные настройки</li></ul>'
+'</td><td id="lpcool">'
+'</td><td id="pravcool">'
+'<h5>Задачи</h5>'
+'<li><a>Последнее значение цикла</a><br>'
+'let i=3;<br>'
+'while(i){<br>'
	+'alert(i--);<br>'
+'} <i>//3,2,1-(последнее)</i><br></li><li>'
+'<a>что выведет while?</a><br>'
+'&ensp;1. Префиксный ++i<br>' 
+'let i=0;<br>'
+'while(++i<5)alert(i); <i>//1,2,3,4</i><br>'
+'&ensp;2. Постфиксный i++<br>'
+'let i=0;<br>'
+'while(i++<5)alert(i); <i>//1,2,3,4,5</i><br></li><li>'
+'<a>Что выведет for</a><br>'
+'&ensp;1. постфиксная<br>'
+'for(let i=0;i<5;i++)alert(i); <i>//0,1,2,3,4</i><br>'
+'&ensp;2. префиксная<br>'
+'for(let i=0;i<5;++i)alert(i); <i>//0,1,2,3,4</i> (то-же самое у меня)</i><br>'
+'<ol><i>И это нормально, ведь алгоритм следующий:</i><br>'
+'<li>Выполнить единожды присваивание i=0 перед чем-либо(начало).</li>'
+'<li>Проверить условие i<5</li>'
+'<li>Если true - выполнить тело цикла alert(i),затем i++.<i>(тут разницы нет, i++ или ++i)</i></li>'
+'</ol>'
+'<a>Введите чётные числа от 2 до 10</a><br>'
+'for(let i=2;i<=10;i++){if(i%2==0)alert(i);} <i>//2,4,6,8,10</i><br></li><li>'
+'<a>Замените for на while</a><br>'
+'for(let i=0;i<3;i++){alert(`number ${i}!`);} <i>//number 0,1,2</i><br>'
+'let i=0;while(i<3){alert(i++);} <i>//0,1,2</i><br><li><li>'
+'<a>Повторять цикл, пока ввод верен</a><br>'
+'let x;<br>'
+'do{<br>'
	+'x=prompt("chislo >100 ",0);<br>'
+'}while(x<=100&&x); <i>//пока больше ста не введёшь, не успокоится</i><br></li><li>'
+'<a>Простые числа</a><br>'
+'let n=0;<br>'
+'do{<br>'
	+'if(n%2!=0&&n%3!=0){alert(n)};<br>'
	+'n++;<br>'
+'}while(n<=10); <i>//1,5,7</i><br>'
+'</li></td></table>'
document.write(zikl);
//prom,prod,zikl
